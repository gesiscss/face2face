...
def create_network_from_data(Data, replace_attr = False, label=None):
    ...
    if label is None:
        ...
        df_edge = Data.interaction.groupby(["i", "j"]).size().reset_index(name="EdgeWeight")
        ...
        df_merge_1 = Data.interaction.merge(metadata, left_on=["i"], right_on=["ID"], how="inner")
        interaction = df_merge_1.merge(metadata, left_on=["j"], right_on=["ID"], how="inner")
        interaction = interaction[["Time", "i", "j"]]

        G = nx.Graph()
        G.add_nodes_from(metadata["ID"].values)
        df_edge = interaction.groupby(["i", "j"]).size().reset_index(name="EdgeWeight")
        for col, row in df_edge.iterrows():
            G.add_edge(row["i"], row["j"])
...
def hopping_time_networks(Data, minutes=1000):
    r"""Create multiple Networkx Graphs and DataFrames
        ...
        Examples
        --------
        In this example the full dataframe got splitted in dataframes and networks with timewindows of 40 seconds
        (2/3 minutes). The Output in this case descripes the network and the dataframe for the first 40 seconds in the
        original dataframe.
    ...
    for i in range(1, len(time_stamp_split_list)):
        for j in unique_timestamp_list:
            if time_stamp_split_list[i] > j >= time_stamp_split_list[i-1]:
                marker_timestamp_list.append([j, time_stamp_split_list[i]])
    timestamp_marker_df = pd.DataFrame(marker_timestamp_list, columns=["Time", "TimeGroup"])
...
def sliding_time_networks(Data, slide=1, interval=1000):
    ...
    for i in time_stamp_split_list:
        for j in unique_timestamp_list:
            if i[1] > j >= i[0]:
                marker_timestamp_list.append([j, i[2]])
...
def event_time_networks(Data, events):
    ...
    for i in events:
        for j in unique_timestamp_list:
            if i[2] >= j > i[1]:
                marker_timestamp_df.append([j, i[0]])
    marker_timestamp_df = pd.DataFrame(marker_timestamp_df, columns=["Time","TimeGroup"])
